        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-semibold leading-tight text-gray-900">
                    {{ __('Approve :file', ['file' => $masterlist->file_name]) }}
                </h2>
            </div>
            <a href="{{ route('chairman.masterlists.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-md border border-emerald-700/20 bg-white px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm transition hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-700 focus:ring-offset-2">
                {{ __('Back to Queue') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-5 flex flex-wrap gap-2">
                <a href="{{ route('chairman.masterlists.show', $masterlist) }}" class="rounded-md px-3 py-2 text-sm font-semibold {{ $activeStatus === '' ? 'bg-emerald-800 text-white' : 'bg-white text-emerald-800 ring-1 ring-emerald-700/20' }}">
                    {{ __('All') }}
                </a>
                @foreach ($verificationStatuses as $status)
                    <a href="{{ route('chairman.masterlists.show', [$masterlist, 'status' => $status]) }}" class="rounded-md px-3 py-2 text-sm font-semibold {{ $activeStatus === $status ? 'bg-emerald-800 text-white' : 'bg-white text-emerald-800 ring-1 ring-emerald-700/20' }}">
                        {{ Str::headline($status) }}
                    </a>
                @endforeach
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-600">{{ __('Coordinator') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-600">{{ __('Chairman Action') }}</th>
                            </tr>
                        </thead>
                            ...
                            @forelse ($records as $record)
                                ...
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        ...
                                        <span class="mt-2 inline-flex rounded-md px-2.5 py-1 text-xs font-semibold {{ $verificationClass }}">
                                    ...
                                    <td class="min-w-80 px-6 py-4 text-sm">
                                        @if ($canEdit)
                                            <form method="POST" action="{{ route('chairman.masterlists.records.update', [$masterlist, $record]) }}" class="space-y-3">
                                                @csrf
                                                @method('PATCH')

                                                <select name="chairman_status" class="block w-full rounded-md border-emerald-900/20 bg-white text-sm text-slate-900 shadow-sm focus:border-emerald-700 focus:ring-emerald-700">
                                                    <option value="approved" @selected($record->chairman_status === 'approved')>{{ __('Approve') }}</option>
                                                    <option value="rejected" @selected($record->chairman_status === 'rejected')>{{ __('Reject') }}</option>
                                                </select>

                                                <textarea name="remarks" rows="2" class="block w-full rounded-md border-emerald-900/20 bg-white text-sm text-slate-900 shadow-sm focus:border-emerald-700 focus:ring-emerald-700" placeholder="{{ __('Remarks required when rejecting') }}">{{ old('remarks', $record->remarks) }}</textarea>
